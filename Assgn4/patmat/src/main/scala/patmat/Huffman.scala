...
object Huffman {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if(trees.length < 2) trees
      ...
      (forknode::trees.tail.tail).sortWith((p1: CodeTree, p2: CodeTree) => weight(p1) < weight(p2))
    }
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def findNextLetter(itree: CodeTree, bits: List[Bit]): (Char, List[Bit]) = {
      itree match {
        case Fork(l, r, chs, w) => {
          if(bits.head == 0) findNextLetter(l, bits.tail)
          else findNextLetter(r, bits.tail)
        }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = {
    table.filter((elem: (Char, List[Bit])) => elem._1 == char).head._2
  }

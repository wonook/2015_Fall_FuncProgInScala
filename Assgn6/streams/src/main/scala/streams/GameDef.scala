...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     */
    def neighbors: List[(Block, Move)] = (this.left, Left)::(this.right, Right)::(this.up, Up)::List((this.down, Down))

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter((e: (Block, Move)) => e._1.isLegal)

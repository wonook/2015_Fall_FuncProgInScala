...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'def :: (Symbol(name) :: params) :: body :: rest :: Nil =>
      eval('def :: Symbol(name) :: ('lambda :: params :: body :: Nil) :: rest :: Nil, env)
    case 'case :: expr :: matches =>
      val fstcarmatches = matches.head match {
        case hd::tl => hd
        case _ => sys.error("check your arguments")
      }
      val sndcarmatches = matches.head match {
        case hd::tl => tl.head
        case _ => sys.error("check your arguments")
      }
      eval(
        'if :: {if(matches.tail == Nil) 1 else 0} :: // see if it's else
          sndcarmatches ::
          ('if :: ('= :: expr :: fstcarmatches :: Nil) ::
            sndcarmatches ::
            ('case :: expr :: matches.tail) :: Nil) :: Nil, env.extend("else", env))
//      'if (= nil ('cdr matches))
//        ('car 'cdr 'car matches)
//        ('if (= expr ('car 'car matches))
//          ('car 'cdr 'car matches)
//          ('case expr ('cdr matches)))
